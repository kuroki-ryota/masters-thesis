...
I would also like to thank Daniel Misselbeck-Wessel for pointing out that my result is relevant to the problems in Lombardi's list.
...
Our constructive proof provides a solution to two questions \cite{Wer17,Arr21} on MathOverflow and two problems \cite[{}1.1, 1.2]{Lom23} in Lombardi's list of open problems,
which has been posted to the Google Groups forum named ``constructivenews.''

Our proof of the general Nullstellensatz implicitly uses the idea of the entailment relation of prime ideal and the entailment relation of maximal ideal \cite{SW21}, although familiarity with entailment relations is not necessary to understand our proof (see \cref{dynamical} for details).
...
Several classical proofs are known for the general Nullstellensatz \cite[Theorem 4.19]{Eis95}, \cite[Theorem 15.36]{AK13}.
Our constructive proof is based on one of the standard proofs that can be found in \cite{Eme}.
The most important step in our proof is the constructive generalization (\cref{Emerton}) of a key lemma \cite[Lemma 6]{Eme} in the classical proof.
...
The foundations of constructive mathematics include predicative systems such as Constructive Zermelo--Frankel set theory (CZF) and Martin-Löf type theory (MLTT), as well as impredicative systems such as Intuitionistic Zermelo--Frankel set theory (IZF) and the Calculus of Inductive Constructions (CIC).
The main difference is that impredicative systems admit the axiom of power set, while predicative systems do not.
Our constructive proof can be formalized in impredicative systems and also in some predicative systems, such as MLTT with universes and CZF.
The key point is the definition of Jacobson rings; the definition used in this thesis contains a quantification over all ideals of a ring.
This definition works in impredicative systems and in some predicative foundations where we can quantify over a power class.
However, it does not work in other predicative foundations that cannot handle power classes at all, such as MLTT without universes.
...
\end{itemize}
Together with the known constructive fact that every zero-dimensional ring is a Jacobson ring, these results show that there are many examples of Jacobson rings even in constructive algebra.
% に加えて

There are other theorems concerning Jacobson rings for which we should provide constructive proofs.
The most important theorems on Jacobson rings are the general Nullstellensatz, which is discussed in this thesis, and the following form of Zariski's lemma:
...
The main difficulty is that maximal ideals appear explicitly in the statement of the theorem.
From a constructive point of view, we believe that one should first reformulate the statement without referring to maximal ideals, but it is not clear how the idea of entailment relations can be used to do this.
We note that this problem appears on Lombardi's list of open problems \cite[{}1.4]{Lom23}.
Wiesnet \cite{Wie21,Wie25} has obtained partial results in the case where $A$ is a discrete field or $\Z$, but the general case is still open.
...
The theory of finitely Jacobson rings is not well developed, and its investigation is also left for future work.
\begin{thebibliography}{99}
% delete irrelevant references
\bibitem[AK13]{AK13} Altman, A., Kleiman, S. (2013). \emph{A Term of Commutative Algebra}, Worldwide Center of Mathematics, LLC.
